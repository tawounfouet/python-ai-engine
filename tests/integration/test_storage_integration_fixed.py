...
class TestStorageInteroperability:
    ...
    def _migrate_storage_data(self, source: InMemoryStorage, target: SQLiteStorage) -> None:
    ...
    def test_storage_consistency_across_implementations(self) -> None:
        ...
        provider.settings.temperature = 0.7
        ...
        assert memory_provider.default_model == sqlite_provider.default_model == provider.default_model
    ...
    def test_concurrent_storage_operations(self) -> None:
        ...
        # Créer des providers pour les agents
        ...
        memory_storage.save_provider(provider1)
        sqlite_storage.save_provider(provider2)
        
        # Créer des agents avec des IDs différents
        agent1 = Agent(name="Agent Memory", slug="agent-memory", provider_id=provider1.id)
        agent2 = Agent(name="Agent SQLite", slug="agent-sqlite", provider_id=provider2.id)
...
class TestStorageEdgeCases:
    ...
    def test_large_data_handling(self) -> None:
        ...
        # Créer un agent
        agent = Agent(name="Bulk Agent", slug="bulk", provider_id=provider.id)
        storage.save_agent(agent)
    ...
    def test_unicode_and_special_characters(self) -> None:
        ...
        provider = LLMProviderConfig(
            ...
            provider_type=ProviderType.OPENAI,
            default_model="gpt-4",
            description="Émojis: 🚀🎯 Unicode: café naïve résumé",
        )
        storage.save_provider(provider)
        ...
        assert "🚀🎯" in retrieved_provider.description
